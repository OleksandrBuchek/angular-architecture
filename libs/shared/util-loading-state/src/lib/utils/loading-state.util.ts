...
import { DataLoadingState, LoadedWithData, LoadingState, Loaded, CombinedData } from '../models';
import { isLoadedWithData, isLoaded, isFailed, isLoading } from './check-status.util';
...
export const combine = (states: LoadingState[]): LoadingState => {
...
const getCombinedData = <T extends Record<string, DataLoadingState<any>>>(statesMap: T): CombinedData<T> => {
  return objectEntries(statesMap).reduce(
    (acc, [key, loadingState]) => ({ ...acc, [key]: (loadingState as LoadedWithData<T>)['data'] }),
    {} as CombinedData<T>,
  );
};

export const combineWithData = <T extends Record<string, DataLoadingState<any>>>(
  states: T,
  extraStates: LoadingState[] = [],
): DataLoadingState<CombinedData<T>> => {
  const combinedState = combine([...objectValues(states), ...extraStates]);
