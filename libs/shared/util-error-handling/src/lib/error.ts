...
const hasMessage = (error: unknown): error is { message: string } => {
  return isObject(error) && 'message' in error && typeof (error as { message: unknown }).message === 'string';
};

export const getErrorMessageFromObject = (error: Record<string, unknown>): string | undefined => {
  if (hasMessage(error)) {
...
export class PGError<T = HttpErrorResponse> extends Error {
  ...
  constructor(error: T | PGError<T>) {
    super('');

    this.origin = error instanceof PGError ? error.origin : error;
